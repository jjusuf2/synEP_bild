...
if run_number is None:
    save_folder = Path(f'/mnt/md0/jjusuf/bild/final_profiles_save_all_20260430/{condition_name}_{delta_t}s')
else:
    save_folder = Path(f'/mnt/md0/jjusuf/bild/final_profiles_save_all_20260430/{condition_name}_{delta_t}s_{run_number}')
...
def calculate_and_save_BILD_result(args):
    ...
    try:
        ...
        print('   trajectory failed', flush=True)
...
with Pool(nproc) as p:
    for i in range(num_chunks):
        print(f'starting chunk {i+1} of {num_chunks} [{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}]', flush=True)
